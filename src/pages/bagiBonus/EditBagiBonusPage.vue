...
const handlePostBagiBonus = async () => {
  ...
  return await putBagiBonus(id, data)
      .then(() => {
        notify({
          type: 'success',
          title: 'Berhasil',
...
const fetchBagiBonusById = async () => {
  return await getBagiBonusById(id)
      .then((res: any) => {
        pembayaran.value = res.data.jumlah_pembayaran;
        form.value = res.data.bagi_bonus_item.map((item: any) => {
          return {
            id: item.id,
            value: item.total_persentase,
            label: item.nama,
            nama: item.nama.toLowerCase().replace(/ /g, '_'),
          }
        })
